...
add_survival_status <- mutate(titanic_df, survival_status = ifelse(Survived == 0, "Did not survive",
                                                            ifelse(Survived == 1, "Survived", "no")))
...
lived_df <- add_survival_status %>%
     group_by(survival_status, Sex, Embarked) %>%
       summarize(
         survival_percentage = n() / nrow(add_survival_status)
            ) %>%
